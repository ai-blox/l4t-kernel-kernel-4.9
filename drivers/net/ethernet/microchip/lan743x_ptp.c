...
void lan743x_ptp_isr(void *context)
{
	...
	if (ptp_int_sts & PTP_INT_BIT_TX_TS_) {
		// toussi ptp_schedule_worker(ptp->ptp_clock, 0);
		//put the above back !!!  
...
static void lan743x_ptp_sync_to_system_clock(struct lan743x_adapter *adapter)
{
	...
	//toussi ktime_get_clocktai_ts64(&ts);
	ktime_get_ts64(&ts);
...
int lan743x_ptp_open(struct lan743x_adapter *adapter)
{
	...
	//toussi ptp->ptp_clock_info.adjfine = lan743x_ptpci_adjfine;
	...
	// toussi ptp->ptp_clock_info.do_aux_work = lan743x_ptpci_do_aux_work;
...
int lan743x_ptp_ioctl(struct net_device *netdev, struct ifreq *ifr, int cmd)
{
	...
	case HWTSTAMP_TX_ONESTEP_SYNC:
		...
		break;
	//toussi case HWTSTAMP_TX_ONESTEP_P2P:
	//	ret = -ERANGE;
	//	break;
	default:
		netif_warn(adapter, drv, adapter->netdev,
