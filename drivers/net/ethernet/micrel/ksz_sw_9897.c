...
dbg_msg(" cached.xmii[%d]: %02x"NL, sw->HOST_PORT - sw->phy_port_cnt, sw->cached.xmii[sw->HOST_PORT - sw->phy_port_cnt]);
	}
 ...
 */
	...
	/* [ KSZ8567R Silicon Errata and Data Sheet Clarification ] : 
	* Module 3: Default RGMII ingress timing does not comply with the RGMII specification
	* DESCRIPTION
	* The RGMII defining document specifies a typical data-to-clock setup time into the receiver (switch signals TXD6_[3:0],
	* TX_ER6 and TX_EN6 to TX_CLK6) of 1.8ns. However, port 6 requires additional setup time in order to avoid ingress
	* data errors on this interface. Refer to the data sheet for details.
	* This issue does not occur on port 7, and there are no timing issues with the MII and RMII modes.
	* END USER IMPLICATIONS
	* Careful analysis of the RGMII timing must be performed, considering the timing of both connected devices, and relative
	* signal delay times on the PCB.
	* Work around
	* Additional PCB trace delay may be needed on the TX_CLK6 clock signal into port 6 of the switch. Another option
	* is to set the ingress delay bit [4] in register 0x6301. Refer to the data sheet for timing details.
	*/
	if(sw->chip_id == KSZ8567_SW_CHIP) {
		port_r(sw, port_6, REG_PORT_XMII_CTRL_1, &data);
		data = data | PORT_RGMII_ID_IG_ENABLE;
		port_w(sw, port_6, REG_PORT_XMII_CTRL_1, data);
		sw->cached.xmii[0] |= data;
		dbg_msg("cached.xmii[0]: %02x"NL, sw->cached.xmii[0]);
	}

	/* [ KSZ8567R Silicon Errata and Data Sheet Clarification ] : 
	* Module 9: No Pause frames are generated for ingress rate limiting with an EEE link
	* DESCRIPTION
	* When an Energy Efficient Ethernet (EEE) link is established with another device, and ingress rate limiting is set up, the
	* port may not generate Pause frames in response to ingress traffic exceeding the rate limit. It also assumes that the
	* Ingress Rate Limit Flow Control Enable bit in register 0xN403 has been set.
	* END USER IMPLICATIONS
	* If Pause frames are not generated by the switch, then the link partner will not be able to regulate the rate at which it
	* sends traffic, making ingress rate limiting ineffective. Note that when the flow control function is enabled for ingress rate
	* limiting, the ingress port will not drop packets when the rate limit is exceeded - it relies only on flow control for limiting
	* the ingress rate.
	* Work around
	* The problem can be resolved by writing to the following three global registers. Note that these registers may not
	* be documented in the data sheet.
	* Global registers:
	* [addr] [data]
	* 0x03C0 0x4090
	* 0x03C2 0x0080
	* 0x03C4 0x2000
	*/
	sw->reg->w16(sw, REG_SW_EEE_QM_CTRL__2, 0x4090);
	/* Note for global register 0x03C2 in this fix: 
	* Another fix has set register 0x03C2 to 0x0040 as below
	* =====================================================
	* THa  2015/10/01
	* Increasing wait time in this register avoids the PTP transmit problem.
	*   sw->reg->w16(sw, REG_SW_EEE_TXQ_WAIT_TIME__2, 0x0040);
	* ======================================================
	* So not sure what value should be written to register 0x03C2. Just keep it unchanged
	*/
	sw->reg->w16(sw, 0x03C4, 0x2000);
}  /* sw_set_global_ctrl */
...
dbg_msg(" %04x=%04x"NL, 0x20, val[0]);
...
#ifdef SETUP_PHY_NEW
	   /* [ KSZ8567R Silicon Errata and Data Sheet Clarification ] :
		* Module 2: Transmit waveform amplitude can be improved (100BASE-TX, 10BASE-T, 10BASE-Te)
		* DESCRIPTION
		* The transmit waveform amplitude can be improved for 10BASE-T, 10BASE-Te and 100BASE-TX.
		* END USER IMPLICATIONS
		* With the default settings, the waveform amplitude may be outside the specifications in some corner case conditions,
		* and the transmitter may not be fully compliant with the IEEE standard. This may degrade performance under some conditions.
		* Work around
		* If AVDDH = 2.5V (supports 10BASE-Te, not 10BASE-T), write to the following MMD registers for each PHY port
		* [1-5]:
		* [MMD] [register] [data]
		* 0x1C 0x4 0x00D0
		* If AVDDH = 3.3V (supports 10BASE-T, not 10BASE-Te), write to the following MMD registers for each PHY port
		* [1-5]:
		* [MMD] [register] [data]
		* 0x1C 0x9 0xE214
		* 0x1C 0x4 0x00D0
	   ...
	   /* [ KSZ8567R Silicon Errata and Data Sheet Clarification ] :
		* Module 4: 100BASE-TX Energy Efficient Ethernet (EEE) is subject to link drop
		* DESCRIPTION
		* Infrequent link drop errors can occur on 100BASE-TX EEE links with long cable length and certain link partners. Separately, data errors may occur at the link partner.
		* END USER IMPLICATIONS
		* By default, EEE is enabled in each switch PHY. If EEE is also enabled in the link partner, then EEE is activated. If the
		* link partner does not support EEE, then EEE will not be activated on the link.
		* With default settings, the EEE power saving feature can result in occasional data loss or infrequent temporary link down
		* conditions.
		* Work around
		* All 100BASE-TX EEE link problems are solved by performing the following register writes. Refer to the MMD section of the data sheet for examples of how to access the indirect PHY MMD registers.
		* Global register:
		* [addr] [data]
		* 0x03C2 0x0080
		* MMD registers:
		...
		* 1 0xC9 0x0180
		* 1 0xC7 0x81E0
		* A second alternative is to disable EEE. This is done by writing the following register:
		* MMD register:
		* [MMD] [register] [data]
		* 7 0x3C 0x0000
		*
		...
		*/
		...
		port_mmd_write(sw, port, MMD_DEVICE_ID_DSP, 0xC9, val, 1);
		val[0] = 0x81E0;
		port_mmd_write(sw, port, MMD_DEVICE_ID_DSP, 0xC7, val, 1);
#else
/*
 * THa  2016/10/20
 ...
 */
		...
		* DESCRIPTION
		* The power supply current specifications in the data sheet are based on the following register settings. Without these
		* register changes, the AVDDH current is approximately 21% greater, and total chip power is approximately 7% greater
		* when all PHY ports are linked at 100Mb/s.
		* END USER IMPLICATIONS
		* If the following register settings are not made, the chip can dissipate more power than indicated in the data sheet for the
		* AVDDH power rail. This means that under the worst case conditions (all PHY ports linked at 100 Mb/s), the junction
		* temperature may approach or exceed 125°C when operating at the maximum ambient temperature. Device and system
		* thermal analysis should use the increased AVDDH current value if the following register settings are not made. The voltage regulator supplying AVDDH must also have adequate current capacity.
		* Work around
		* Write to the following MMD registers for each PHY port [1-5]:
		* [MMD] [register] [data]
		* 0x1C 0x13 0x6EFF
